...
def test_all_cases(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4,
                   save_result=True, test_save_path=None, preproc_func=None, metric_detail=0, nms=0):
    total_metric = 0.0
    ...
    for image_path in loader:
        ...
        if metric_detail:
            print('%02d,\t%.5f, %.5f, %.5f, %.5f' % (
                index, single_metric[0], single_metric[1], single_metric[2], single_metric[3]))

        total_metric += np.asarray(single_metric)

        if save_result:
            nib.save(nib.Nifti1Image(prediction.astype(np.float32),
                                     np.eye(4)), test_save_path + "%02d_pred.nii.gz" % index)
            nib.save(nib.Nifti1Image(image[:].astype(np.float32), np.eye(
                4)), test_save_path + "%02d_img.nii.gz" % index)
            nib.save(nib.Nifti1Image(label[:].astype(np.float32), np.eye(
                4)), test_save_path + "%02d_gt.nii.gz" % index)
        index += 1
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                dis_to_mask = tf.math.sigmoid(-1500 * y1_tanh)
